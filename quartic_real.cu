...
/*
Hmm this is using lots of recursion... to handle cases
*/
...
CUDA_BOTH
static int solve_real_poly_reverse(const double* poly, double* results);
...
{
    // TODO: reaarange to avoid recursion but still handle degree sliding

    const double a = poly[degree];

    if (degree == 0) return 0;

    if (a == 0) return solve_real_poly(degree - 1, poly, results);

    if (degree > MAX_DEGREE) return -1;

    if (degree > 2 && stableness_score(poly[degree], poly[degree - 1]) > stableness_score(poly[0], poly[1])) 
    {
        return solve_real_poly_reverse( degree, poly, results ); 
    }


    double normalized_poly[MAX_DEGREE + 1];
    for (int i = 0; i < degree; ++i) normalized_poly[i] = poly[i] / a;
...
CUDA_BOTH
int solve_real_poly_reverse(int degree, const double* poly, double* results) 
{
    double rev_poly[MAX_DEGREE + 1];
    int num_results;
    for (i = 0; i <= degree; ++i) rev_poly[i] = poly[degree - i];

    num_results = solve_real_poly(degree, rev_poly, results);

    for (i = 0; i < num_results; ++i) results[i] = 1.0 / results[i];
    return num_results;
}
